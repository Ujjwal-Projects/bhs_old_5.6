...
if($_GET['status']=='delete'){
  //rint_r($_GET);die;
   $obj->deleteData(bhs_performance,"id='".$_GET['id']."'");
}
if($_POST['Save']){
	pre($_POST);
	if($_POST['title']!=""){
			list($fileName,$error)=$obj->uploadFile('attachment', 'performance', 'gif,jpg,jpeg,png');
			if($fileName) 
			{ 
				$_POST['attachment'] = $fileName;
			}
		
		$insert_id=$obj->insertData(bhs_performance,$_POST);	
		$_SESSION['succ_msg'] = 'PERFORMANCE Added Successfully.';
		unset($_POST);
		$obj->reDirect('performance.php');
	}else{
		$_SESSION['fail_msg'] = 'Fillup All data correctly.';
		unset($_POST);
		$obj->reDirect('performance.php');		
	}
    ...
    <section class="content">
	   ...
	   <?php if($_SESSION['succ_msg']){?>
          ...
          <div class="box box-primary">
            <div class="box-header with-border">
              <h3 class="box-title">Add PERFORMANCE</h3>
            </div>
            <!-- /.box-header -->
            <!-- form start -->
	   ...
	   <div class="box-body">
		<table id="example1" class="table table-bordered table-striped">
				...
				<?php
					$i=1;
					$res = $obj->selectData(bhs_performance,"","status<>'deleted'");
                ...
                <tr>
                  ...
                  <td><?php if($data['attachment']!=""){?><img  src="performance/<?php echo $data['attachment'];?>" width="200" height="150"><?php } ?></td>
                  <td><a href="status_change.php?id=<?php echo $data['id']?>&status=<?php echo $status_change_to?>&return_url=<?php echo base64_encode($_SERVER['REQUEST_URI'])?>&tbl=<?php echo bhs_performance;?>&fld=id"><?php echo $activeimg;?></a>
                  <a class="btn btn-danger" href="performance.php?id=<?php echo $data['id']?>&status=delete">Delete</a></td>
