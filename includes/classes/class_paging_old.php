...
define('PREV','&lt;');
define('NEXT','&gt;');
define('NO_RESULT','<div class="'.CSSNFE.'">No Record Found!</div><br>');//change text for no result
...
class pagingRecords
{
	...
	function setPagingParam($pagingType="",$pgshownum="",$limnum="",$pgfl="",$pgnp="",$ht="")
	{
		$this->googlepaging=0;
		$this->yahoopaging=0;
		$this->normalpaging=0;
	...
	function runQueryPaging($sql,$pageno="",$lnkParam="",$lnkScr="")
	{
		...
		{
			//echo $this->SQL."<br>";
			if($this->getPos($this->SQL,"DISTINCT"))
			{
				// do nothing sql same
			}
			else
	...
	function pagewise($numrows,$pageno=1,$lnkParam="",$limit="",$lnkScr="")
	{
		...
		//$pgStr.="<tr><td width=\"100%\" valign=\"top\" align=\"right\" class=\"".CSS."\" style=\"float:left\">";
		$st_page=1;
		$end_page=$pages;
		...
		if($selectedPg!=1)
		{
			...
			{	
				if($this->htpaging){
					$pgStr.="<li class='page-item'><a class='".CSSFL."' href=\"".FURL.$this->htpaging."/page/1/".$lnkParam."\" title=\"FIRST\">".$this->firstText."</a></li>&nbsp;  ";
				} else {
					$pgStr.="<li class='page-item'><a class='".CSSFL."' href=\"".$this->linksrc."?pageno=1".$lnkParam."\" title=\"FIRST\">".$this->firstText."</a></li>&nbsp;  ";
				}
			}
			if($this->nextprev==1)
			{
				...
				if($this->htpaging){
					$pgStr.="<li class='page-item'><a class='".CSSNP."' href=\"".FURL.$this->htpaging."/page/".$newpageno."/".$lnkParam."\" title=\"PREV\">".$this->prevText."</a></li>&nbsp;  ";
				} else {
					$pgStr.="<li class='page-item'><a class='".CSSNP."' href=\"".$this->linksrc."?pageno=".$newpageno.$lnkParam."\" title=\"PREV\">".$this->prevText."</a></li>&nbsp;  ";
				}
			}
		}
		if(!$this->simplepaging && !$this->dropdownpaging)
		{
			for ($i=$st_page; $i<=$end_page; $i++)
			{
				if($selectedPg == $i)
				{
					$pgStr.="<li class='page-item active'><a class='active'>".$i."</a></li> &nbsp; ";
				}
				else
				{	
					if($this->htpaging){
						$pgStr.="<li class='page-item'><a class='".CSSPN."' href=\"".FURL.$this->htpaging."/page/".$i."/".$lnkParam."\">".$i."</a></li>";
					} else {
						$pgStr.="<li class='page-item'><a class='".CSSPN."' href=\"".$this->linksrc."?pageno=".$i.$lnkParam."\">".$i."</a></li>";
					}
				}
			}
		}
		...
		if($selectedPg!=$pages)
		{
			if($this->nextprev==1)
			{
				$newpageno = abs($selectedPg+1);
				if($this->htpaging){
					$pgStr.="<li class='page-item'><a class='".CSSNP."' href=\"".FURL.$this->htpaging."/page/".$newpageno."/".$lnkParam."\" title=\"NEXT\">".$this->nextText."</a></li>";
				} else {
					$pgStr.="<li class='page-item'><a class='".CSSNP."' href=\"".$this->linksrc."?pageno=".$newpageno.$lnkParam."\" title=\"NEXT\">".$this->nextText."</a></li>";
				}
			}
			if(!$this->normalpaging && $this->firstlast==1)
			{
				if($this->htpaging){
					$pgStr.=" <li class='page-item'><a class='".CSSFL."' href=\"".FURL.$this->htpaging."/page/".$pages."/".$lnkParam."\" title=\"LAST\">".$this->lastText."</a></li>";
				} else {
					$pgStr.=" <li class='page-item'><a class='".CSSFL."' href=\"".$this->linksrc."?pageno=".$pages.$lnkParam."\" title=\"LAST\">".$this->lastText."</a></li>";
				}
			}
		}
		//$pgStr.="</td></tr>";
